...
/* */
Esfera::Esfera() :
	Esfera(CGameWindow::DEFAULT_WINDOW_WIDTH, CGameWindow::DEFAULT_WINDOW_HEIGHT)
{
}

/* */
Esfera::Esfera(int window_width, int window_height) :
...
{
	cout << "Constructor: Esfera(int window_width, int window_height)" << endl;
...
Esfera::~Esfera()
{
	...
	if (m_sphereVertexArrayObject > 0)
	{
...
/* */
void Esfera::update(double deltaTime)
{
...
void Esfera::createSphereGeometry()
{
	float radio = 2.0f;
	bool loaded = false;
	...
	for (int e = 0; e < aros; e++)
	{
		...
		for (int i = e * verticesCircle * 3; i < e*verticesCircle * 3 + (verticesCircle * 3); i++)
		{
			...
			float newX = (float)(radio * cosf(angle2) * cosf(angle));
			float newZ = (float)(radio * cosf(angle2)* sinf(angle));
			float newY = (float)(radio * sinf(angle2));
	...
	/*
	//ordenar para acercarlos
	double minDistancia = 100000000;
	for (int i = 0; i < verticesTotales * 3; i++)
	{
		// Last i elements are already in place
		for (int j = i; j < verticesTotales * 3; j++)
		{
			double distancia = sqrtl((vData[j] - vData[i]) * (vData[j] - vData[i]) + (vData[j + 2] - vData[i + 2]) * (vData[j + 2] - vData[i + 2]));

			if (distancia < minDistancia && vData[j+1] == vData[i+1])
			{
				swap(&vData[i+3], &vData[j]);
				swap(&vData[i+4], &vData[j]);
				swap(&vData[i+5], &vData[j]);
	...
	*/
	...
	for (int i = 0; i < m_numFacesSphere; i++)
	{
		// Vertex 1
...
void Esfera::normcrossprod(float v1[3], float v2[3], float out[3])
{
...
/* */
void Esfera::normalize(float v[3])
{
...
/* */
void Esfera::onMouseMove(float deltaX, float deltaY)
{
